...
- [ ] GitHub Actions workflow with the three-compiler matrix, `-Wall -Wextra -Wpedantic -Werror` and `/W4 /WX`, running the full test suite and building the example
- [ ] Workflow job running the fuzz target for one minute under ASan and UBSan on a supporting compiler
- [ ] CMake install and export produce a config package; a separate consumer test project uses `find_package` and links both targets
- [ ] `OSCPM_VERSION` macro and CMake project version agree; semantic versioning documented
